...
export function setupMyRentalsCommand(
    ...
    bot.command('my_rentals', async (ctx) => handleMyRentals(ctx, userService, rentRequestService));
...
export async function handleMyRentals(ctx: Context, userService: UserService, rentRequestService: RentRequestService) {
    if (!ctx.from) return;
